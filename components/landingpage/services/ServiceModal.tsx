...
export default function ServiceModal(props: any) {
  return (
    ...
    >
      {props.infos}
      <div className="absolute p-6 grid grid-rows-6 grid-row bg-white border-8 border-primary w-4/5 h-4/5 sm:w-2/5 sm:h-3/5 left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 z-50">
        <span
          className="absolute top-2 right-2 m-4 cursor-pointer"
          onClick={() => props.setShowModal(false)}
        >
          <CrossIcon width={20} height={20} color="#193BF5" />
        </span>
        <div className="row-span-1 h-auto sm:m-auto">
          <h1 className="text-3xl font-GilroyBold mb-4 text-primary h-10">
            {props.title}
          </h1>
        </div>
